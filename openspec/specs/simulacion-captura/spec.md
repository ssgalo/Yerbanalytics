# simulacion-captura Specification

## Purpose
Panel de cámara dentro de la app de simulación: emite órdenes de prueba y carga diagnósticos a mano usando los mismos endpoints que usará la plataforma en producción, sin superficie de API propia.

## Requirements

### Requirement: Panel de cámara dentro de la app de simulación
La app de simulación SHALL incluir una sección de cámara para probar el dispositivo de
captura, servida en el mismo puerto aislado que el resto del simulador. Esta sección NO SHALL
exponerse en el dashboard de monitoreo.

#### Scenario: La sección vive en el simulador
- **WHEN** el usuario abre la app de simulación
...
El panel SHALL mostrar el estado del dispositivo de cámara —operativo, intermitente o fuera de
servicio— y el instante de su última señal de vida, y SHALL permitir generar un código de
...
- **WHEN** el usuario carga estado, severidad y nivel de confianza sobre una captura recibida y
  confirma
...
- **WHEN** el usuario carga un diagnóstico y luego abre la vista de Diagnósticos de IA
