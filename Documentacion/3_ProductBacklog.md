...
| --- | --- | --- | --- | --- | --- |
| #HU-001 | Cliente | iniciar sesión de forma segura en la plataforma | acceder a la información de mi producción protegida | Baja | To Do |
| #HU-002 | Cliente | visualizar las métricas de sustrato y ambiente | conocer el estado fisiológico de mis plantines | Media | To Do |
| #HU-003 | Cliente | que el sistema capture periódicamente las variables del sustrato y ambiente | tener una visión de la evolución de los plantines | Alta | To Do |
...
| #HU-005 | Cliente | consultar los diagnósticos emitidos con su nivel de confianza y severidad | entender el estado sanitario de mi producción | Media | To Do |
| #HU-006 | Cliente | que el sistema ejecute riego solo en los sectores que lo requieran | mantener condiciones óptimas del sustrato sin desperdiciar agua | Alta | To Do |
| #HU-007 | Cliente | que el sistema dosifique nutrientes y pesticidas sólo en los sectores afectados | tratar el problema sin desperdiciar insumos | Alta | To Do |
| #HU-008 | Cliente | que el sistema despliegue o retire la mediasombra de forma gradual | ejecutar el proceso de rustificación lumínica controlada | Alta | To Do |
...
| #HU-010 | Cliente | recibir alertas cuando el sistema detecta una anomalía o ejecuta una acción crítica | estar informado sin recorrer el vivero | Media | To Do |
| #HU-011 | Cliente | consultar el historial completo de acciones ejecutadas junto a la condición que las originó | auditar y respaldar mis controles de calidad | Media | To Do |
| #HU-012 | Cliente | ver el estado del sector después de cada acción ejecutada | evaluar su efectividad | Media | To Do |
| #HU-013 | Cliente | que el sistema siga operando sin conexión | no perder información | Alta | To Do |
...
| #HU-016 | Cliente | exportar reportes de trazabilidad | respaldar auditorías y controles de calidad | Baja | To Do |
| #HU-017 | Cliente | acceder a la plataforma desde cualquier dispositivo | supervisar mi producción desde donde esté | Media | To Do |
...
*Como Cliente quiero que el sistema capture periódicamente las variables del sustrato y ambiente para tener una visión de la evolución de los plantines.*
...
*Como Cliente quiero consultar los diagnósticos emitidos con su nivel de confianza y severidad para entender el estado sanitario de mi producción.*
...
*Como Cliente quiero que el sistema ejecute riego solo en los sectores que lo requieran para mantener condiciones óptimas del sustrato sin desperdiciar agua.*
...
| CA-02 | un pronóstico de lluvia inminente por encima del umbral configurado | se evalúa una orden de riego | el sistema suspende o pospone el riego para evitar saturación hídrica |
| CA-03 | una acción de riego ejecutada | finaliza | el sistema registra el evento con sector, duración y condición que lo originó |

---

### Historia de Usuario: #HU-007 *(CORE)*
*Como Cliente quiero que el sistema dosifique nutrientes y pesticidas solo en los sectores afectados para tratar el problema sin desperdiciar insumos.*
...
| CA-02 | una acción de dosificación | se ejecuta | el sistema registra insumo, dosis, sector y diagnóstico que la originó |
| CA-03 | un sector sano | se evalúan acciones | el sistema no aplica ningún insumo sobre ese sector |
...
| CA-03 | una caída o falta de respuesta de la API climática | el sistema necesita decidir | opera con las mediciones provistas por los sensores, sin realizar acciones preventivas hasta que recuperar la conexión |
...
*Como Cliente quiero ver el estado del sector después de cada acción ejecutada para evaluar su efectividad.*
...
*Como Cliente quiero que el sistema siga operando sin conexión para no perder información.*
...
| CA-02 | la recuperación de conectividad | se restablece la conexión | el sistema sincroniza los datos almacenados con el servidor sin duplicarlos ni perderlos |
| CA-03 | la sincronización completada | el Cliente accede a la plataforma | visualiza los eventos ocurridos durante el período offline con su marca temporal real |
...
- Las funcionalidades implementadas fueron probadas y funcionan de acuerdo con lo esperado.
- No se detectaron errores críticos o pendientes asociados a la historia.
- Se verificó que los cambios no afecten el comportamiento de funcionalidades existentes.
- Las pruebas automatizadas definidas para la historia se ejecutan correctamente.
- Se realizó una validación funcional completa de la historia integrando los servicios necesarios.
- El código y la documentación asociados a la historia pasaron por una revisión entre pares.
- La implementación fue integrada correctamente en la rama destinada a validación del proyecto.
- Se corroboró el cumplimiento de los criterios de aceptación establecidos para la historia.
